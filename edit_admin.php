...
<?php include('header.php'); ?>
    ...
    <div class="page-title">
        <div class="title_left">
            <h3>
                <small><a href="">Home</a> / <a href="admin_profile.php">Profile</a> /</small> Edit
            </h3>
        </div>
    </div>
    ...
    <div class="row">
        <div class="col-md-12 col-sm-12 col-xs-12">
            <div class="x_panel">
                <div class="x_title">
                    <h2><i class="fa fa-pencil" style="padding-top: 10px;"></i> Edit</h2>
                ...
                <div class="x_content">
                    ...
                    <form method="post" enctype="multipart/form-data" class="form-horizontal form-label-left">
                        ...
                        <div class="form-group">
                            ...
                            <div class="col-md-3">
                                <input type="text" value="<?php echo $row['lastname']; ?>" name="lastname"
                                       id="last-name2" required="required" class="form-control col-md-7 col-xs-12">
                            </div>
                        </div>
                        <div class="form-group">
                        ...
                        <div class="form-group">
                            ...
                            <div class="col-md-3">
                                <input type='tel' value="<?php echo $row['contact']; ?>" autocomplete="off"
                                       maxlength="11" name="contact" id="last-name2"
                                       class="form-control col-md-7 col-xs-12">
                            </div>
                        </div>
                        <div class="form-group">
                        ...
                        <div class="form-group">
                            ...
                            <div class="col-md-3">
                                <input type="text" value="<?php echo $row['LTIM_mail']; ?>" name="ltim_mail"
                    ...
                    if (isset($_POST['update'])) {
                        ...
                        if (!preg_match('/^[a-zA-Z\s]{3,50}$/', $firstname)) {
                            echo "<script>alert('Invalid First Name! Only letters are allowed.'); window.location='edit_admin.php?admin_id=.$id'</script>";
                        } elseif (!preg_match('/^[a-zA-Z\s]{1,50}$/', $lastname)) {
                            echo "<script>alert('Invalid Last Name! Only letters are allowed.'); window.location='edit_admin.php?admin_id=.$id'</script>";
                        } elseif (($currentYear - $dobYear) < 18) {
                            echo "<script>alert('Age should be 18 or older!'); window.location='edit_admin.php?admin_id=.$id'</script>";
                        } elseif (($currentYear - $dobYear) > 70) {
                            echo "<script>alert('Age should be less than 70!'); window.location='edit_admin.php?admin_id=.$id'</script>";
                        } elseif (!preg_match('/^[A-Za-z0-9]{2,4}$/', $grade)) {
                            echo "<script>alert('Invalid Grade format!'); window.location='edit_admin.php?admin_id=.$id'</script>";
                        } elseif (!preg_match('/^[A-Za-z\s]{10,50}$/', $role)) {
                            echo "<script>alert('Invalid Role format! Only letters are allowed.'); window.location='edit_admin.php?admin_id=.$id'</script>";
                        } elseif (!preg_match('/^\d{10}$/', $contact)) {
                            echo "<script>alert('Invalid Contact format! Please enter a 10-digit number.'); window.location='edit_admin.php?admin_id=.$id'</script>";
                        } elseif (!preg_match('/^[a-zA-Z0-9\s\-,]+$/i', $address)) {
                            echo "<script>alert('Invalid Address format! Please enter a valid address.'); window.location='edit_admin.php?admin_id=.$id'</script>";
                        } elseif (!filter_var($ltim_mail, FILTER_VALIDATE_EMAIL)) {
                            echo "<script>alert('Invalid LTIM Mail format! Please enter a valid email address.'); window.location='edit_admin.php?admin_id=.$id'</script>";
                        } elseif (!filter_var($pluto_mail, FILTER_VALIDATE_EMAIL)) {
                            echo "<script>alert('Invalid Pluto Mail format! Please enter a valid email address.'); window.location='edit_admin.php?admin_id=.$id'</script>";
                        } else {
                      ...
                      contact= '$contact', address='$address', LTIM_mail='$ltim_mail', Pluto_mail='$pluto_mail' WHERE ps_no = $id") or die(mysqli_error());
                                echo "<script>alert('Successfully Updated Admin Info!'); window.location='admin_profile.php'</script>";
                            }
                        }
                    }
                    ?>
